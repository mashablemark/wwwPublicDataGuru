...
var ownershipForms= ['3', '3/A', '4', '4/A', '5', '5/A'];
...
function ingestNextQuarter(processControl){
    if(!processControl.current){ //just starting
        ...
        if(processControl.current.year + processControl.current.q/10>processControl.end.year + processControl.end.q/10){
            logEvent("finished 345 scrape", processControl.quartersProcessed + " quarters processed");
...
function processQuarterlyIndex(processControl, callback){
    ...
    logEvent('345 quarter ingest' ,'fetching ' + idxUrl, true);
    request(idxUrl, function (error, response, indexBody) {
        ...
        function next345(){  //the master index has refereence to all submission; just get the next 3/4/5 ownership or false if none left to process
            if(!lineNum) return false;
            while(lineNum<lines.length){
                lineNum++;
                var lineParts = lines[lineNum].split('|');
                if(lineParts.length!=5) return false;
                if(ownershipForms.indexOf(lineParts[linePartsIndex.FormType])!=-1) {
                    //console.log('processing line number '+ lineNum);
                    return lineNum;
                }
            }
            return false;
