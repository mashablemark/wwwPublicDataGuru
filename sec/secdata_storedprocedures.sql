DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `eftsTypingHintsLookup`(IN `typed_letters` VARCHAR(100))
    NO SQL
BEGIN

  DECLARE maxLookupDepth int DEFAULT 15;
  DECLARE typeAheadDepth int;
  DECLARE lookupKey varchar(100);


  DECLARE cikMatch int DEFAULT null;
  DECLARE cikMatchName varchar(500) DEFAULT null;
  DECLARE cikMatchTickers varchar(500) DEFAULT null;

  #check for exact CIK match
  IF(TRIM(typed_letters) = CEIL(TRIM(typed_letters)) AND CEIL(TRIM(typed_letters))>1500) THEN
  	SELECT e.cik, e.name, coalesce(e.ticker, GROUP_CONCAT(t.ticker))
    INTO cikMatch, cikMatchName, cikMatchTickers
    FROM efts_entities e LEFT OUTER JOIN edgar_tickers t on e.cik = t.cik
    WHERE e.cik = CEIL(TRIM(typed_letters))
    GROUP BY e.cik;
  END IF;

  #find nearest branch in tree
  set lookupKey = left(REPLACE(REPLACE(LTRIM(typed_letters),'.','_'),' ','_'), maxLookupDepth);
  set typeAheadDepth = least(char_length(lookupKey), maxLookupDepth);
  getTypingHints: LOOP

    select count(*) into @rows
    	from search_typing_hints h
        where h.keys_typed = left(lookupKey, typeAheadDepth) COLLATE latin1_general_ci;
   	IF @rows = 1 THEN
        select
            typed_letters as request,
            char_length(typed_letters) as requestDepth,
            left(lookupKey, typeAheadDepth) as branchKey,
            typeAheadDepth as branchDepth,
            json_top_matches as branchTopHints,
            cikMatch,
            cikMatchName,
            cikMatchTickers
        from search_typing_hints h where h.keys_typed = left(lookupKey, typeAheadDepth) COLLATE latin1_general_ci;
      LEAVE getTypingHints;
    END IF;

    SET typeAheadDepth = typeAheadDepth - 1;  # next level down in hints tree
    IF typeAheadDepth > 0 THEN
      ITERATE getTypingHints;
    END IF;
  ...
  END LOOP getTypingHints;

END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `eftsTypingHintsMaker`()
    NO SQL
BEGIN
#executes in 1 min 22s on 1 year of data (128,541 distinct CIKs) (re)creating search_typing_hints up to 10 keys contining 174,025 hint rows with zero down time:
#note: 10 years of disseminations have 382,403 distinct CIKs

#keys_typed_count    hints count    terminal hint count
#1                   37
#2                   996             129
#3                   9392            3512
#4                   24739           11901
...
#9                   23021           10633
#10                  19515           8345
#by end of this proc, entitiesWRK reduced to 42,500 rows by removing entities on terminal branches

#typeAheadMaxDepth = 15 run in 1m35s on 1 year's entities

  DECLARE typeAheadMaxDepth int DEFAULT 15;
  DECLARE typeAheadDepth int DEFAULT 0;
  DECLARE entityCountPerHint int DEFAULT 10;
  DECLARE archivesLengthInDays int DEFAULT DATEDIFF(now(),STR_TO_DATE('1/1/1994', '%m/%d/%Y'));

  DROP TABLE IF EXISTS  search_typing_hints_WRK;
  CREATE TABLE search_typing_hints_WRK (
    keys_typed varchar(255) PRIMARY KEY,
    keys_typed_count tinyint,
    cik_count int NULL,
    cik_sum bigint NULL,
    rank_cutoff double null,
    json_top_matches text NULL
  )
  DEFAULT CHARSET=latin1 COLLATE latin1_general_ci;

  # entitiesWRK not ony contains the cleaned name, it's records are pruned as terminal hint branch are found
  DROP TABLE IF EXISTS  entitiesWRK;
  CREATE TABLE entitiesWRK   # exec time = 9.4 sec
	SELECT
    	e.cik,
       	e.name,
    	e.ticker,
        sum(f.length_indexed * (1-DATEDIFF(now(), s.filedt)/archivesLengthInDays)) as rank
    FROM efts_entities e
      INNER JOIN efts_submissions_entities se ON e.cik=se.cik
      INNER JOIN efts_submissions s ON se.adsh=s.adsh
      INNER JOIN efts_submissions_files f ON se.adsh=f.adsh
    GROUP BY e.cik, e.name, e.ticker;
  ALTER TABLE entitiesWRK ADD PRIMARY KEY (cik);

  #edgar_tickers is a static table; eventually it should use FAST Act data from XBRL
  update entitiesWRK ew,
  (select cik, GROUP_CONCAT(trim(replace(ticker,' ','-')) order by char_length(ticker) ASC SEPARATOR '_') as tickers from edgar_tickers group by cik) et
  set ew.ticker = et.tickers, ew.rank=10*ew.rank #boost tradable entities with ticker symbols
  WHERE ew.ticker is null and ew.cik=et.cik;

  DROP TABLE IF EXISTS  hintPathsWRK;
  CREATE TABLE hintPathsWRK   # exec time = 9.4 sec
        SELECT
            ew.cik,
            lcase(left(REPLACE(REPLACE(trim(ew.name), '.', ''),' ','_'), typeAheadMaxDepth)) as path,
            char_length(left(REPLACE(REPLACE(trim(ew.name), '.', ''),' ','_'), typeAheadMaxDepth)) as path_length,
            ticker as partial_path, # just want a VARCHAR(20); will overwrite the values in loop
            0 as isTicker,
            ew.rank
        FROM entitiesWRK ew
    UNION
        SELECT
            et.cik,
            lcase(left(REPLACE(REPLACE(trim(et.ticker), '.', ''),' ','-'), typeAheadMaxDepth)) as path,
            char_length(left(REPLACE(REPLACE(trim(et.ticker), '.', ''),' ','-'), typeAheadMaxDepth)) as path_length,
            et.ticker as partial_path, # just want a VARCHAR(20); will overwrite the values in loop
            1 as isTicker,
            ew.rank
        FROM edgar_tickers et INNER JOIN entitiesWRK ew on et.cik=ew.cik;

  CREATE INDEX idx_hintpathcik ON hintPathsWRK (cik);

  makeTypeAhead: LOOP
    SET typeAheadDepth = typeAheadDepth + 1;  # next level down in hints tree

    # create the partial ticker and name keys for current type ahead depth
    UPDATE hintPathsWRK  # exec time = 1.0 sec for 70,000 rows
    	SET partial_path = IF(char_length(path) >= typeAheadDepth, left(path, typeAheadDepth), null);
    # prune too shorts
    delete from hintPathsWRK where partial_path is null;

    # insert records for next level of hints (subsequent update will create JSON)
    INSERT INTO search_typing_hints_WRK (keys_typed, keys_typed_count) # exec time = 0.05 sec
      	SELECT DISTINCT partial_path, typeAheadDepth
        FROM hintPathsWRK
        WHERE path_length >= typeAheadDepth;

  	CREATE INDEX idx_part_path_rank ON hintPathsWRK (partial_path, rank); # exec time = 0.2 sec

    # determine the rank cutoff (RANK() statement requires Maria DB 10.2+)
    UPDATE # exec time = 0.6 sec for level 1 = 39 rows; 06 sec for level 2 = 832 rows
    	search_typing_hints_WRK hw,
        (select keys_typed, min(rank) as rank_cutoff
        from (
            select hw.keys_typed, rank, RANK() OVER (PARTITION BY hw.keys_typed ORDER BY pw.rank DESC) as x
            from hintPathsWRK pw INNER JOIN search_typing_hints_WRK hw on hw.keys_typed = pw.partial_path
            #anything smaller already deleted from hintPathsWRK, so this is not needed: where hw.keys_typed_count = typeAheadDepth
        ) all_ranks
        where x <= entityCountPerHint
        GROUP BY keys_typed) rank_cutoffs
    SET hw.rank_cutoff = rank_cutoffs.rank_cutoff
    WHERE hw.keys_typed = rank_cutoffs.keys_typed;

 	UPDATE #exec time = 4.6 sec for level 2 = 835 rows
       search_typing_hints_WRK etaw,
      (SELECT
       	  hit_list.keys_typed,
       	  concat('[', GROUP_CONCAT(hit),']') as hit_array,
          COUNT(DISTINCT cik) as cik_count,
          SUM(DISTINCT cik) as cik_sum
       FROM
          (
            (SELECT # grab direct matches regardless of rank
                taw.keys_typed,
                ew.cik,
                CONCAT('{"CIK":', ew.cik, ',"tickers":', coalesce(concat('"',replace(ew.ticker,'_',', '),'"'),'null'),',"entity":"', ew.name, '"}') as hit
            FROM  hintPathsWRK pw INNER JOIN entitiesWRK ew on pw.cik=ew.cik
            	INNER JOIN search_typing_hints_WRK taw on taw.keys_typed = pw.path
            ORDER BY pw.rank DESC)
          UNION DISTINCT
            (SELECT # add 5 top ranks of partial matches to any direct matches found above (note: it is possible for multiple entities to have the same rank
                taw.keys_typed,
                ew.cik,
                CONCAT('{"CIK":', ew.cik, COALESCE(CONCAT(', "tickers":', '"',REPLACE(ew.ticker,'_',', '),'"'),''),',"entity":"', ew.name, '"}') as hit
            FROM hintPathsWRK pw INNER JOIN entitiesWRK ew on pw.cik=ew.cik
             INNER JOIN search_typing_hints_WRK taw on taw.keys_typed = pw.partial_path
            WHERE rank_cutoff<=pw.rank
            ORDER BY pw.rank DESC)
        ) hit_list
        GROUP BY  hit_list.keys_typed
      ) hit_arrays
    SET
    	etaw.json_top_matches = hit_arrays.hit_array,
        etaw.cik_count = hit_arrays.cik_count,
        etaw.cik_sum = hit_arrays.cik_sum
    WHERE etaw.keys_typed = hit_arrays.keys_typed;

    DELETE
    FROM hintPathsWRK
    USING hintPathsWRK INNER JOIN search_typing_hints_WRK hw ON hintPathsWRK.cik=hw.cik_sum and hintPathsWRK.partial_path = hw.keys_typed
    WHERE hw.cik_count=1 AND hintPathsWRK.isTicker=0;

    DROP INDEX idx_part_path_rank ON hintPathsWRK; # exec time = 0.005 sec

    IF typeAheadDepth < typeAheadMaxDepth THEN  #depth of the hints decision tree
      ITERATE makeTypeAhead;
    END IF;
    LEAVE makeTypeAhead;

  END LOOP makeTypeAhead;


  START TRANSACTION;
    DROP TABLE IF EXISTS  search_typing_hints;
    RENAME TABLE  search_typing_hints_WRK TO  search_typing_hints;
  COMMIT;
