...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateFrame`(IN `TAG` VARCHAR(255), IN `UOM` VARCHAR(20), IN `CCP` VARCHAR(9), IN `FRAMEJSON` MEDIUMTEXT, IN `TSTAMP` TIMESTAMP, IN `QTRS` SMALLINT)
    NO SQL
BEGIN
...
IF char_length(FRAMEJSON) = 0 THEN  
    ...
    group by f.tag, f.uom, f.ccp;
 	IF FRAMEEXISTS=1 THEN #frame exists
    	select json, tstamp from frames f where f.tag=TAG and f.uom=UOM and f.ccp=CCP;
    ELSE  #frame DNE; create and then return skeleton frame
    	INSERT into frames (tag, uom, ccp, qtrs, json, pts, tstamp) 
        select TAG, UOM, CCP, QTRS, 
    	   concat('{"tag":"',TAG,'","ccp":"',CCP, '","uom":"',UOM,'","qtrs":',QTRS,'"label":"',replace(st.label, 
           '"', '\"'),'","description":"', replace(st.description, '"', '\"'),'pts": 0, , data": []}') as json, 
           0, now()
        from standardtags st where st.tag=TAG;
    	select json, tstamp from frames f where f.tag=TAG and f.uom=UOM and f.ccp=CCP;
    END IF;
ELSE  #update request
    ...
    IF FRAMEEXISTS=1 THEN
    	update frames f set f.json=FRAMEJSON where f.tag=TAG and f.uom=UOM and f.ccp=CCP;
